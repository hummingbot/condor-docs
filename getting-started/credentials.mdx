...
<AccordionGroup>
  <Accordion title="Invalid API key">
    ...
    - Adding a credential runs a live balance check before saving it, so a rejected key fails with the **exchange's own error text** rather than a generic message — read it for the actual cause (bad key, wrong permissions, expired key, clock skew, and so on)
  </Accordion>
