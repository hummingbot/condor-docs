---
title: "Installation"
description: "Deploy Condor and Hummingbot API"
---

import FeedbackSurvey from "/snippets/feedback-survey.mdx";

## Prerequisites

<AccordionGroup>
  <Accordion title="System Requirements">
    | Component | Minimum | Recommended |
    |-----------|---------|-------------|
    | OS | Linux x64 or ARM (Ubuntu 20.04+), macOS | Ubuntu 22.04 LTS |
    | Memory | 4 GB RAM | 8 GB RAM |
    | Storage | 10 GB SSD | 20 GB SSD |
    | CPU | 2 vCPUs | 4 vCPUs |
  </Accordion>
  <Accordion title="Cloud Server Options">
    Any cloud provider works. Popular choices:
    - **AWS EC2**: t3.medium or larger
    - **Google Cloud**: e2-medium or larger
    - **Digital Ocean**: Basic Droplet with 4GB RAM
    - **Hetzner**: CX21 or larger

    Configure security group to allow ports 22 (SSH) and 8000 (API).
  </Accordion>
</AccordionGroup>

## Install Condor

From an **empty directory** on the machine where you want Condor (and optionally the Hummingbot API on the same host), run:

```bash
curl -fsSL https://raw.githubusercontent.com/hummingbot/deploy/main/setup.sh | bash
```

For API-only installs, manual `git clone` setup, and more detail, see [Installing Condor](/getting-started/installing).

The installer will prompt for:

- Your **Telegram bot token** (see [Create a Telegram Bot](/getting-started/installing#create-a-telegram-bot) or [@BotFather](https://t.me/botfather))
- Your **authorized Telegram user ID(s)**
- Whether to **configure and launch Hummingbot API** with Docker—answer **yes** if you are deploying **Condor and the API on the same machine** locally. Have **[Docker](https://docs.docker.com/get-docker/)** installed and running first. If the API runs on another server, you can skip this step and add that API URL later in Telegram under **`/servers`**.

## Verify Installation

Open your Telegram bot and send `/start`. You should see the main menu.

> **Note:** After a successful start, **admins** should receive **"Condor is online and ready."** in Telegram. If not, see [Troubleshooting](/getting-started/troubleshooting).

## Access Points

| Service | URL | Description |
|---------|-----|-------------|
| API | `http://localhost:8000` | REST API endpoints |
| Swagger UI | `http://localhost:8000/docs` | Interactive API documentation |

## Managing Services

**After the deploy installer:** Condor runs in a **tmux** session named `condor`.

```bash
tmux attach -t condor
tmux kill-session -t condor   # stop Condor
```

Upgrade later from the directory that contains `condor`:

```bash
curl -fsSL https://raw.githubusercontent.com/hummingbot/deploy/main/setup.sh | bash -s -- --upgrade
```

**Hummingbot API:** From the `hummingbot-api` directory: `docker compose ps` and `docker compose logs -f`.

**Manual / dev run** (`make run` from a git clone): stop with **Ctrl+C** in that terminal.

## Next Steps

<CardGroup cols={2}>
  <Card title="Getting Started" icon="rocket" href="/getting-started/overview">
    Connect Telegram, add keys, and start using Condor
  </Card>
  <Card title="Trading Agents" icon="robot" href="/trading-agents/overview">
    Learn about the agent framework
  </Card>
</CardGroup>

Got Condor running? Tell us how install went and what you need next.

<FeedbackSurvey />
